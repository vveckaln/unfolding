...
int main(int argc, char * argv[])
{
  ...
  unsigned long size;
  float *binsunsplit = optimisev2(_th2, stdev_gen, size, 0.8);
  printf("unsplit bins\n");
  for (unsigned char bind = 0; bind < size; bind ++)
    printf("%u %f\n", bind, binsunsplit[bind]);
  writetofile(binsunsplit, size, file);
  printf("split bins \n");
  float * bins = splitbins(binsunsplit, size);
  for (unsigned char bind = 0; bind < 2 * size - 1; bind ++)
    printf("%u %f\n", bind, bins[bind]);
  //return 0;
  // TH1 * proj = _th2 -> ProjectionY("pxt", 1, 1);
  // double max = proj -> GetMaximum();
  // double mean = proj -> GetMean();
  // TF1 * f1 =  new TF1("f1", "gaus", 0.0, TMath::Pi());
  // printf("mean %f %f \n", 0.0, max);
  // double sigma = 1.0;
  // f1 -> SetParameters(max, 0.0, sigma);
  // TFitResultPtr r = proj -> Fit("f1",  "S", "", 0.0, TMath::Pi());
  // printf("amplitude %f error %f\n", r -> Parameter(0), r -> ParError(0));
  // printf("mean %f error %f\n", r -> Parameter(1), r -> ParError(1));
  // printf("sigma %f error %f\n", r -> Parameter(2), r -> ParError(2));
...
float * optimisev2(TH2 * h2, double * sigmas, unsigned long & size, float factor)
{
  ...
  while (edge <= xaxis -> GetBinUpEdge(nbins))
      ...
      if (binfraction > 0.25 * factor)
	{
      ...
      binfraction += h2 -> Integral(bind1, bind2, 1, h2 -> GetNbinsX())/h2 -> Integral();
