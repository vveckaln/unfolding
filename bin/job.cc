...
int main(int argc, char * argv[])
{
 ...
 if (levelind == OPT)
      ...
      if (string(binning_method).compare("SIGMA") == 0)
	{
	  ...
	  subdir += "/" + sigmadirstr;
	  binfilename = TString("/eos/user/v/vveckaln/bins/") + 
	...
	getchar();*/
      chisto = new CompoundHistoUnfolding(TString(tag_opt[levelind]) + "_" + sampletag + "_" + method, chutitle, 2 * (size - 1) , xbins, size - 1, ybins);
  ...
  if (not presampleTTJets)
	...
	method + bintag + "/save.root";
      TFile  * input_file_MC13TeV_TTJets = TFile::Open(TString("root://eosuser.cern.ch/") + input_file_name_MC13TeV_TTJets);
  ...
  if (not calculate_bins)
    {
      ...
      for (vector<HistoUnfolding *> :: iterator it = chisto -> GetLevel(IN) -> _vsyshistos.begin(); it != chisto -> GetLevel(IN) -> _vsyshistos.end(); it ++)
	{
