...
int main(int argc, char * argv[])
{
  ...
  for (unsigned char obsind = 0; obsind < Nobsact; obsind ++)
      ...
      for (unsigned char bindx = 0; bindx <= h2migration[obsind] -> GetNbinsX() + 1; bindx ++)
	{
      ...
      // getchar();
      TUnfoldDensity unfold(h2migration[obsind], TUnfold::kHistMapOutputVert, TUnfold::kRegModeCurvature, TUnfold::kEConstraintArea, TUnfoldDensity::kDensityModeBinWidthAndUser);                                          
      unfold.SetInput(hinputnominal);
      unfold.DoUnfold(0.0);
      houtputnominal[obsind] = unfold.GetOutput((TString("nominal_") + tag_jet_types_[jets[obsind]] + "_" + observables[obsind]).Data());
      ...
      for (unsigned char it = 0; it < Nit; it ++)
	{
	  ...
	  TUnfoldDensity unfoldit(h2migration[obsind], TUnfold::kHistMapOutputVert/*, TUnfold::kRegModeCurvature, TUnfold::kEConstraintArea, TUnfoldDensity::kDensityModeBinWidthAndUser*/);                
	  //	  hinputreplica -> Scale(hinputnominal -> Integral()/hinputreplica -> Integral());
      	  unfoldit.SetInput(hinputreplica);
	  
	  
	  unfoldit.DoUnfold(0.0);
	  TH1 * houtput = unfoldit.GetOutput("output");
	  //houtput -> Print("all");
  ...
  TH2 * hcorr = new TH2I("corr", "Correlation matrix (ROOT::TUnfold)", bincount, 0.0, bincount, bincount, 0.0, bincount);
  ...
  cematrix -> SaveAs("corr_schmitt.png");
  for (unsigned char obsind = 0; obsind < Nobsact; obsind ++)
    {
      // delete [] xbins;
      // delete [] ybins;
  ...
  for (unsigned char bind = 1; bind <= input -> GetNbinsX(); bind ++)
    {
      ...
      std::uniform_int_distribution<int> distribution(0, 2 * input -> GetBinError(bind));
      const unsigned long nevents =  input -> GetBinContent(bind) - input -> GetBinError(bind) + distribution(generator);//random -> Integer(2 * input -> GetBinError(bind)) ;
      //printf("%lu\n", nevents);
