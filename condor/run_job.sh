...
echo "Parameters to executable:"
echo "$JETIND $CHARGEIND $SAMPLETAG $OBSERVABLE $LEVELIND $METHOD $PRESAMPLETTJETS $BINNINGMETHOD $BINS $SFACTOR $REG"

job=""
if [ "$METHOD" == "nominal" ] || [ "$METHOD" == "cflip" ];
then
    job="job"
else
    job="jobGEN"
...
$UNFOLDINGLIB/$job $JETIND $CHARGEIND $SAMPLETAG $OBSERVABLE $LEVELIND $METHOD $PRESAMPLETTJETS $BINNINGMETHOD $BINS $SFACTOR $REG
#$gdb -batch --args $UNFOLDINGLIB/job $JETIND $CHARGEIND $SAMPLETAG $OBSERVABLE $LEVELIND $METHOD $PRESAMPLETTJETS $BINNINGMETHOD $BINS $SFACTOR

EXIT_CODE_BIN=$?
echo "EXIT_CODE_BIN " $EXIT_CODE_BIN >&2
eos root://eosuser.cern.ch// mkdir -p $OUTPUTDIR/$JOBTAG
